...
export default async function handler(
  ...
  try {
    const details = await (media_type === 'tv'
      ? moviedb.tvInfo({ id, language: 'en-US' })
      : moviedb.movieInfo({ id, language: 'en-US' }));
    const keywordResults = await (media_type === 'tv'
      ? moviedb.tvKeywords({ id })
      : moviedb.movieKeywords({ id }));
    // @ts-ignore TODO: look into this
    const keywords = keywordResults[
      'results' in keywordResults ? 'results' : 'keywords'
    ] as Keyword[];
    const similar = await getMediaSuggestions(details, keywords);
    const resultsWithImages = similar?.results?.map((r) => ({
      ...r,
    ...
    const results = {
      ...
      details,
      keywords,
      similar: { ...similar, results: resultsWithImages },
      error: undefined,
    };
    // @ts-ignore TODO: look into this
...
async function getMediaSuggestions(
  mediaDetails: MovieResponse | ShowResponse,
  keywords: Keyword[]
) {
  try {
